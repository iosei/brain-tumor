...
class GlobalConfig:
    ...
    pretrained_model_path = 'C://Users//NEW//Desktop//FedTumor//FedTumor//BRATS//Data//Brats2020//brats2020logs//brats2020logs//fedprox//last_epoch_model_dp.pth'
...
class DeiTBlock(nn.Module):
    def __init__(self, in_channels, out_channels, img_size=224):
        ...
        # DeiT model with a specified configuration
        self.deit_model = timm.create_model('deit_base_patch16_224', pretrained=False)
        self.deit_model.head = nn.Identity()  # Remove the classification head
    ...
    def forward(self, x):
        ...
        # Apply the 2D DeiT model to each slice
...
class UNet3d(nn.Module):
    def __init__(self, in_channels, n_classes, n_channels):
        ...
        self.deit_block = DeiTBlock(8 * n_channels, 8 * n_channels)
    ...
    def forward(self, x):
        ...
        # Apply DeiTBlock instead
        deit_output = self.deit_block(x5)


        mask = self.dec1(deit_output, x4)
...
class Trainer:
    """
    Factory for training process with Differential Privacy using Opacus.
    """
    ...
    def __init__(self,
        ...
        self._initialize_data_loaders(dataset, path_to_csv, fold, batch_size)
        print(type(dataset))
        ...
        if self.enable_dp:
            ...
            self.net, self.optimizer, self.dataloaders["train"] = self.privacy_engine.make_private(
    ...
    def _initialize_data_loaders(self, dataset, path_to_csv, fold, batch_size):
        """Initialize dataloaders and assign to self.dataloaders."""
        self.dataloaders = {
    ...
    def _compute_loss_and_outputs(self, images: torch.Tensor, targets: torch.Tensor):
        """Compute loss and forward pass outputs."""
    ...
    def _do_epoch(self, epoch: int, phase: str):
        """Perform training or validation epoch."""
        ...
        with BatchMemoryManager(
            ...
            max_physical_batch_size=self.accumulation_steps,
            optimizer=self.optimizer
        ) as memory_safe_loader:
            for itr, data_batch in enumerate(memory_safe_loader):
    ...
    def run(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            # Display privacy budget for Differential Privacy
            if self.enable_dp:
                epsilon, best_alpha = self.privacy_engine.get_epsilon(delta=self.privacy_engine.target_delta)
                print(f"(ε = {epsilon:.2f}, δ = {self.privacy_engine.target_delta}) for α = {best_alpha}")
