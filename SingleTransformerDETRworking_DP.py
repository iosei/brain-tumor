    ...
    def plot_cube(self,
        ...
        with plt.style.context("dark_background"):
            ...
            if make_gif:
                ...
                xmin, xmax = -5, 5  # Replace with actual min and max of your data
                ymin, ymax = -1, 1  # Replace with actual min and max of your data
                zmin, zmax = -1, 1  # Replace with actual min and max of your data
                ...
                # Rotate the view and save each frame as an image
                for angle in tqdm(range(0, 360, 5)):
                    ...
                    if len(images) > 0 and img.shape != images[0].shape:
                        img = np.resize(img, images[0].shape)

                    images.append(img)

                    # Optional: Remove the saved images if you don't need them
                    # os.remove(fname)
...
def merging_two_gif(path1: str, path2: str, name_to_save: str):
    ...
    Parameters:
        ...
        name_to_save: nam88e for saving new GIF.
    """
...
def accuracy_metric(probabilities: torch.Tensor,
    ...
    """
    Calculate accuracy for the data batch.
    Params:
...
class Meter:
    '''Factory for storing and updating IoU, Dice, and Accuracy scores.'''

    def __init__(self, threshold: float = 0.5):
        """
        Initialize Meter with optional threshold.
        :param threshold: The threshold for binary classification.
        """
        self.threshold: float = threshold
        self.dice_scores: list = []  # To store dice scores of each batch
        self.iou_scores: list = []   # To store IoU scores of each batch
        self.accuracy_scores: list = []  # To store accuracy scores of each batch

    def update(self, logits: torch.Tensor, targets: torch.Tensor):
        """
        Update the metrics with current logits (model outputs) and targets (ground truth).
        :param logits: Model outputs before sigmoid (raw predictions).
        :param targets: Ground truth segmentation masks.
        """
        probs = torch.sigmoid(logits)  # Apply sigmoid to get probabilities from logits
        dice = dice_coef_metric(probs, targets, self.threshold)  # Calculate Dice score
        iou = jaccard_coef_metric(probs, targets, self.threshold)  # Calculate IoU score
        accuracy = accuracy_metric(probs, targets, self.threshold)  # Calculate accuracy
    ...
    def get_metrics(self) -> np.ndarray:
        """
        Get the average Dice, IoU, and Accuracy scores across all accumulated batches.
        :return: A tuple containing average Dice, IoU, and Accuracy scores.
        """
        dice = np.mean(self.dice_scores)  # Calculate the mean Dice score
        iou = np.mean(self.iou_scores)   # Calculate the mean IoU score
        accuracy = np.mean(self.accuracy_scores)  # Calculate the mean Accuracy
...
def add_laplace_noise(tensor, epsilon, sensitivity):
    """
    Add Laplace noise to a tensor.
    :param tensor: The tensor to which noise will be added.
    :param epsilon: Privacy budget parameter.
    :param sensitivity: Sensitivity of the operation.
    :return: Noisy tensor.
    """
    noise = torch.tensor(
        np.random.laplace(0, sensitivity / epsilon, tensor.size()),
        dtype=torch.float32,
...
class Trainer:
    ...
    Args:
        ...
        num_epochs: num weights updates for all data.
        ...
        accuracy_scores: dict for storing lists with accuracy scores for each phase.
    """
    ...
    def __init__(self,
        ...
        self.accuracy_scores = {phase: [] for phase in self.phases}  # Added accuracy tracking
    ...
    def _compute_loss_and_outputs(self, images: torch.Tensor, targets: torch.Tensor):
        ...
        # Calculate accuracy
        preds = (logits > 0.5).float()  # Binarize predictions
    ...
    def _do_epoch(self, epoch: int, phase: str):
        ...
        epsilon = 0.05  # Example privacy budget
        sensitivity = 1.0  # Example sensitivity, depending on task and data
            ...
            if phase == "train":
                ...
                for param in self.net.parameters():
                    if param.grad is not None:
                        param.grad = add_laplace_noise(param.grad, epsilon, sensitivity)
        ...
        epoch_dice, epoch_iou, epoch_accuracy = meter.get_metrics()  # Adjusted unpacking
        self.losses[phase].append(epoch_loss)
        self.dice_scores[phase].append(epoch_dice)
        self.jaccard_scores[phase].append(epoch_iou)
        self.accuracy_scores[phase].append(epoch_accuracy)  # Append accuracy
    ...
    def _plot_train_history(self):
        ...
        labels = [
            ...
            f"train dice score {self.dice_scores['train'][-1]} val dice score {self.dice_scores['val'][-1]}",
            f"train jaccard score {self.jaccard_scores['train'][-1]} val jaccard score {self.jaccard_scores['val'][-1]}",
    ...
    def _save_train_history(self):
        ...
        logs_ = [self.losses, self.dice_scores, self.jaccard_scores, self.accuracy_scores]
        log_names_ = ["_loss", "_dice", "_jaccard", "_accuracy"]
        ...
        pd.DataFrame(dict(zip(log_names, logs))).to_csv("raw_detection_train_log_single.csv", index=False)
