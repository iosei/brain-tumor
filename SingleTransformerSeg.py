...
class GlobalConfig:
    root_dir = '/BRATS/Data/Brats2020/BraTS2020_TrainingData'
    train_root_dir = '/BRATS/Data/Brats2020/BraTS2020_TrainingData/MICCAI_BraTS2020_TrainingData'
...
def sample_test():
    sample_filename = '/BRATS/Data/Brats2020/BraTS2020_TrainingData/MICCAI_BraTS2020_TrainingData/BraTS20_Training_001/BraTS20_Training_001_flair.nii.gz'
    sample_filename_mask = '/BRATS/Data/Brats2020/BraTS2020_TrainingData/MICCAI_BraTS2020_TrainingData/BraTS20_Training_001/BraTS20_Training_001_seg.nii.gz'
...
class SwinTransformerBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.swin_transformer_2d = timm.create_model('swin_base_patch4_window7_224', pretrained=False)
        self.swin_transformer_2d.head = nn.Identity()  # Remove the classification head

        # Assuming the Swin Transformer does not change spatial resolution,
        # only the number of channels (features).
        #self.conv1x1 = nn.Conv3d(3, out_channels, kernel_size=1)
    ...
    def forward(self, x):
        ...
        # Apply the 2D Swin Transformer to each slice
        # Resize each slice to the expected input size of the Swin Transformer
        x = F.interpolate(x, size=(224, 224), mode='bilinear', align_corners=False)
        x = self.adjust_channels(x)
        x = self.swin_transformer_2d(x)
        # print("shape",x.shape)
        # Reshape back to original dimensions
        x = x.view(b, d, -1, 224, 224)  # -1 for the new channel dimension
...
class UNet3d(nn.Module):
    def __init__(self, in_channels, n_classes, n_channels):
        ...
        self.swin_transformer = SwinTransformerBlock(8 * n_channels, 8 * n_channels)
    ...
    def forward(self, x):
        ...
        swin = self.swin_transformer(x5)

        mask = self.dec1(swin, x4)
...
if __name__ == "__main__":
    ...
    survival_info_df = pd.read_csv(
        '/BRATS/Data/Brats2020/BraTS2020_TrainingData/MICCAI_BraTS2020_TrainingData/survival_info.csv')
    name_mapping_df = pd.read_csv(
        '/BRATS/Data/Brats2020/BraTS2020_TrainingData/MICCAI_BraTS2020_TrainingData/name_mapping.csv')
