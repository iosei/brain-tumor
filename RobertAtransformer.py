...
class RoBERTaBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        # 3D convolution to adapt channels before RoBERTa processing
    ...
    def forward(self, x):
        ...
        x = self.conv1x1(x)
        print(f"Shape after conv1x1: {x.shape}")

        # Reshape the tensor to make it compatible with RoBERTa
        x = x.view(b , d, c* h* w)
        print(f"Shape after view: {x.shape}")

        # Apply the RoBERTa model
        x = self.roberta_model(x)
        print(f"Shape after RoBERTa model: {x.shape}")

        # Reshape back to the original dimensions
        x = x.view(b, d, -1, h, w)
        print(f"Shape after view back: {x.shape}")
...
class UNet3d(nn.Module):
    def __init__(self, in_channels, n_classes, n_channels):
        ...
        self.roberta_transformer = RoBERTaBlock(8 * n_channels, 8 * n_channels)
    ...
    def forward(self, x):
        ...
        swin = self.roberta_transformer(x5)
