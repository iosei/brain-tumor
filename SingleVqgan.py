...
class ViT_VQGAN_Block(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        ...
        # Adjust channels from VQGAN output to desired output channels
        self.output_conv = nn.Conv2d(3, out_channels, kernel_size=1)
...
# Replace ViT-VQGAN block in UNet3d
class UNet3d(nn.Module):
    def __init__(self, in_channels, n_classes, n_channels):
        ...
        # ViT-VQGAN block
        self.vit_vqgan = ViT_VQGAN_Block(8 * n_channels, 8 * n_channels)
    ...
    def forward(self, x):
        ...
        # Apply ViT-VQGAN block
        vit_vqgan = self.vit_vqgan(x5)

        mask = self.dec1(vit_vqgan, x4)
