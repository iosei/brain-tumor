...
class PVTBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.pvt = timm.create_model('pvt_v2_b0', pretrained=False)
        self.pvt.head = nn.Identity()  # Remove the classification head

        # The number of output channels for PVT small is 512
    ...
    def forward(self, x):
        ...
        # Apply the 2D PVT to each slice
        # Resize each slice to the expected input size of the PVT
        x = F.interpolate(x, size=(224, 224), mode='bilinear', align_corners=False)
        x = self.adjust_channels(x)
        x = self.pvt(x)

        # Reshape back to original dimensions
        x= x.view(1,1,1,9, 256)
...
class UNet3d(nn.Module):
    def __init__(self, in_channels, n_classes, n_channels):
        ...
        # PVT block
        self.pvt_block = PVTBlock(8 * n_channels, 8 * n_channels)
    ...
    def forward(self, x):
        ...
        # Apply PVT block
        pvt = self.pvt_block(x5)

        mask = self.dec1(pvt, x4)
...
# Example implementations for DoubleConv, Down, Up, and Out layers
