...
class Image3dToGIF3d:
    ...
    def plot_cube(self,
        ...
        # Add the alpha channel to facecolors based on the cube data
        facecolors[:, :, :, -1] = cube

        # Explode the facecolors array
        facecolors = self._explode(facecolors)

        # Create the filled boolean array based on non-zero values
        filled = facecolors[:, :, :, -1] != 0

        # Expand the coordinates for voxel plotting
        x, y, z = self._expand_coordinates(np.indices(np.array(filled.shape) + 1))

        # Ensure that the shapes are consistent
        assert x.shape == y.shape == z.shape == filled.shape, \
            f"Shape mismatch between voxel coordinates and filled array: x={x.shape}, y={y.shape}, z={z.shape}, filled={filled.shape}"
        assert filled.shape == facecolors.shape[:-1], \
            f"Shape mismatch between filled and facecolors: filled={filled.shape}, facecolors={facecolors.shape}"
        ...
        with plt.style.context("dark_background"):
            ...
            # Plot the voxels using consistent data shapes
...
def compute_scores_per_classes(model, dataloader, classes):
    ...
    model = model.to(device)  # Move the model to the correct device (GPU if available)
    ...
    with torch.no_grad():
        for i, data in enumerate(dataloader):
            ...
            imgs, targets = imgs.to(device), targets.to(device)  # Move data to the same device

            logits = model(imgs)  # Forward pass
            logits = logits.detach().cpu().numpy()  # Move logits back to CPU for metrics calculation
            targets = targets.detach().cpu().numpy()  # Move targets back to CPU for metrics calculation
