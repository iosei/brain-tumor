...
class GlobalConfig:
    ...
    pretrained_model_path = '../BRATS/Data/Brats2020/brats2020logs/brats2020logs/unet/last_epoch_model.pth'
    train_logs_path = '../BRATS/Data/Brats2020/brats2020logs/brats2020logs/unet/train_log.csv'
    ae_pretrained_model_path = '../BRATS/Data/Brats2020/brats2020logs/brats2020logs/ae/autoencoder_best_model.pth'
    tab_data = '../BRATS/Data/Brats2020/brats2020logs/brats2020logs/data/df_with_voxel_stats_and_latent_features.csv'
    seed = 55
...
class MultimodalUNet3d(nn.Module):
    def __init__(self, in_channels_modality1, in_channels_modality2, in_channels_modality3, in_channels_modality4, n_classes, n_channels):
        super().__init__()
        self.in_channels_modality1 = in_channels_modality1
        self.in_channels_modality2 = in_channels_modality2
        self.in_channels_modality3 = in_channels_modality3
        self.in_channels_modality4 = in_channels_modality4
        self.n_classes = n_classes
        self.n_channels = n_channels

        # Define input layers for each modality
        self.conv_modality1 = DoubleConv(in_channels_modality1, n_channels)
        self.conv_modality2 = DoubleConv(in_channels_modality2, n_channels)
        self.conv_modality3 = DoubleConv(in_channels_modality3, n_channels)
        self.conv_modality4 = DoubleConv(in_channels_modality4, n_channels)

        # ... (the rest of the network remains unchanged)

    def forward(self, x_modality1, x_modality2, x_modality3, x_modality4):

        # Process each modality separately
        x1_modality1 = self.conv_modality1(x_modality1)
        x1_modality2 = self.conv_modality2(x_modality2)
        x1_modality3 = self.conv_modality3(x_modality3)
        x1_modality4 = self.conv_modality4(x_modality4)

        # Combine features from all modalities (you can concatenate or apply another operation)
        combined_features = torch.cat([x1_modality1, x1_modality2, x1_modality3, x1_modality4], dim=1)

        # Continue with the rest of the U-Net architecture
        x2 = self.enc1(combined_features)
        x3 = self.enc2(x2)
        x4 = self.enc3(x3)
        x5 = self.enc4(x4)
        ...
        mask = self.dec4(mask, combined_features)  # Note that we use the combined features
        mask = self.out(mask)
...
class Trainer:
    ...
    def _do_epoch(self, epoch: int, phase: str):
        ...
        for itr, data_batch in enumerate(dataloader):
            # Update this part based on the keys present in your data_batch
            if 'image_modality1' in data_batch and 'image_modality2' in data_batch:
                images_modality1 = data_batch['image_modality1']
                images_modality2 = data_batch['image_modality2']
            else:
                # Handle the case where modalities are not found in data_batch
                continue

            targets = data_batch['mask']

            # Call the model's forward method with both modalities
            loss, logits = self._compute_loss_and_outputs(images_modality1, images_modality2, targets)
    ...
    def _plot_train_history(self):
        # Modify this method to display or log performance metrics for each modality.
        # You can create separate plots/logs for each modality's losses and scores.
        data_modality1 = [self.losses_modality1, self.dice_scores_modality1, self.jaccard_scores_modality1]
        data_modality2 = [self.losses_modality2, self.dice_scores_modality2, self.jaccard_scores_modality2]
        colors = ['deepskyblue', "crimson"]
        labels_modality1 = [
            f"""
            train loss {self.losses_modality1['train'][-1]}
            val loss {self.losses_modality1['val'][-1]}
            """,

            f"""
            train dice score {self.dice_scores_modality1['train'][-1]}
            val dice score {self.dice_scores_modality1['val'][-1]} 
            """,

            f"""
            train jaccard score {self.jaccard_scores_modality1['train'][-1]}
            val jaccard score {self.jaccard_scores_modality1['val'][-1]}
            """,
        ]
        labels_modality2 = [
            f"""
            train loss {self.losses_modality2['train'][-1]}
            val loss {self.losses_modality2['val'][-1]}
            """,

            f"""
            train dice score {self.dice_scores_modality2['train'][-1]}
            val dice score {self.dice_scores_modality2['val'][-1]} 
            """,

            f"""
            train jaccard score {self.jaccard_scores_modality2['train'][-1]}
            val jaccard score {self.jaccard_scores_modality2['val'][-1]}
            """,
        ]
        ...
        with plt.style.context("seaborn-dark-palette"):
            ...
            for i, ax in enumerate(axes):
                ax[0].plot(data_modality1[i]['val'], c=colors[0], label="val (Modality 1)")
                ax[0].plot(data_modality1[i]['train'], c=colors[-1], label="train (Modality 1)")
                ax[0].set_title(labels_modality1[i])
                ax[0].legend(loc="upper right")

                ax[1].plot(data_modality2[i]['val'], c=colors[0], label="val (Modality 2)")
                ax[1].plot(data_modality2[i]['train'], c=colors[-1], label="train (Modality 2)")
                ax[1].set_title(labels_modality2[i])
    ...
    def _save_train_history(self):
        # Modify this method to save logs and model weights separately for each modality.
        # You can create separate log files and model weight files for each modality.
        torch.save(self.net_modality1.state_dict(), "last_epoch_model_modality1.pth")
        torch.save(self.net_modality2.state_dict(), "last_epoch_model_modality2.pth")

        logs_modality1 = [self.losses_modality1, self.dice_scores_modality1, self.jaccard_scores_modality1]
        logs_modality2 = [self.losses_modality2, self.dice_scores_modality2, self.jaccard_scores_modality2]

        log_names_modality1 = ["_loss_modality1", "_dice_modality1", "_jaccard_modality1"]
        log_names_modality2 = ["_loss_modality2", "_dice_modality2", "_jaccard_modality2"]

        logs_modality1 = [logs_modality1[i][key] for i in list(range(len(logs_modality1)))
                          for key in logs_modality1[i]
                          ]
        logs_modality2 = [logs_modality2[i][key] for i in list(range(len(logs_modality2)))
                          for key in logs_modality2[i]
                          ]

        log_names_modality1 = [key + log_names_modality1[i]
                               for i in list(range(len(logs_modality1)))
                               for key in logs_modality1[i]
                               ]

        log_names_modality2 = [key + log_names_modality2[i]
                               for i in list(range(len(logs_modality2)))
                               for key in logs_modality2[i]
                               ]

        pd.DataFrame(
            dict(zip(log_names_modality1, logs_modality1))
        ).to_csv("raw_train_log_modality1.csv", index=False)

        pd.DataFrame(
            dict(zip(log_names_modality2, logs_modality2))
        ).to_csv("raw_train_log_modality2.csv", index=False)
...
def train_val_3unet(dataloader):
    ...
    ax.imshow(np.ma.masked_where(mask == False, mask),
              cmap='cool', alpha=0.6)
    #plt.show()
    n_channels = 24  # Specify the common number of channels
    in_channels_modality1 = 4  # Number of channels for modality 1
    in_channels_modality2 = 4
    in_channels_modality3 = 4
    in_channels_modality4 = 4 # Number of channels for modality 2
    n_classes = 3

    nodel = MultimodalUNet3d(in_channels_modality1, in_channels_modality2, in_channels_modality3, in_channels_modality4, n_classes, n_channels).to(device)

    # nodel = MultimodalUNet3d(in_channels=4, n_classes=3, n_channels=24).to(device)
    # %%
...
if __name__ == "__main__":
    ...
    # Dataset DataLoader
    dataloader = get_dataloader(dataset=BratsDataset, path_to_csv='train_data.csv', phase='valid', fold=0,is_check_shapes=False)
