...
class MaeBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # Apply the 2D MAE to each slice
        # Resize each slice to the expected input size of the MAE
        x = F.interpolate(x, size=(224, 224), mode='bilinear', align_corners=False)
        x = self.adjust_channels(x)

        # Flatten the image for MAE input
        x = x.view(b * d, 3, 224 , 224) #.permute(0, 2, 1)  # (batch, sequence_length, channels)

        x = self.mae(x).last_hidden_state
        # Reshape back to original dimensions
        x= x.view(1,192,9, 50, 4)
...
class UNet3d(nn.Module):
    def __init__(self, in_channels, n_classes, n_channels):
        ...
        # MAE Transformer block
        self.mae_transformer = MaeBlock(8 * n_channels, 8 * n_channels)
    ...
    def forward(self, x):
        ...
        x4 = self.enc3(x3)
        x5 = self.enc4(x4)
        # Apply MAE Transformer block
        mae = self.mae_transformer(x5)
        print(mae.shape)
        mask = self.dec1(mae, x4)
