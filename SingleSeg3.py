...
class MultiScaleConv(nn.Module):
    """
    A multi-scale convolutional block with different kernel sizes.
    """
...
class CrossAttention(nn.Module):
    """
    Cross-attention module for combining encoder and decoder features.
    """

    def __init__(self, channels, heads=8, dim_head=64):
        super().__init__()
        self.qkv = nn.Linear(channels, channels * 3)
        self.heads = heads
        self.dim_head = dim_head
        self.rearrange = Rearrange('b c (h w d) -> b (h w d) c', h=heads)

    def forward(self, x, skip):
        qkv = self.qkv(torch.cat([x, skip], dim=1)).chunk(3, dim=-1)
        q, k, v = map(self.rearrange, qkv)
...
class TransformerUNet3d(nn.Module):
    def __init__(self, in_channels, out_channels, features):
        super().__init__()
        self.down1 = MultiScaleConv(in_channels, features)
        self.down2 = MultiScaleConv(features, features * 2)
        self.down3 = MultiScaleConv(features * 2, features * 4)

        # Transformer blocks (replace with efficient transformers if needed)
        self.transformer1 = CrossAttention(features * 4)
        self.transformer2 = CrossAttention(features * 2)

        self.up1 = MultiScaleConv(features * 4, features * 2)
        self.up2 = MultiScaleConv(features * 2, features)
    ...
    def forward(self, x):
        ...
        t1 = self.transformer1(d3, d2)
        u1 = self.up1(F.interpolate(t1, scale_factor=2, mode='trilinear', align_corners=True))

        t2 = self.transformer2(u1, d1)
        u2 = self.up2(F.interpolate(t2, scale_factor=2, mode='trilinear', align_corners=True))
