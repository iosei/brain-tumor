...
class ShowResult:
    ...
    def mask_preprocessing(self, mask, y):
        """
        Test.
        """
        mask = mask.squeeze().cpu().detach().numpy()
        #mask = np.clip(mask, 0, 1)
        mask = (mask >= 0.33).astype(np.int8)
        print(mask)
        mask = np.moveaxis(mask, (0, 1, 2, 3), (0, 3, 2, 1))

        mask_WT = np.rot90(mask[0][:,:,y])
        mask_TC = np.rot90(mask[1][:,:,y])
        mask_ET = np.rot90(mask[2][:,:,y])

        return mask_WT, mask_TC, mask_ET

    def image_preprocessing(self, image, y):
        """
        Returns image flair as mask for overlaping gt and predictions.
        """
        image = image.squeeze().cpu().detach().numpy()
        image = np.moveaxis(image, (0, 1, 2, 3), (0, 3, 2, 1))
        flair_img = np.rot90(image[0][:,:,y])
        return flair_img

    def plot(self, image2, ground_truth, prediction, id):

        i = 95
        image = self.image_preprocessing(image2, i)
        gt_mask_WT, gt_mask_TC, gt_mask_ET = self.mask_preprocessing(ground_truth, i)
        pr_mask_WT, pr_mask_TC, pr_mask_ET = self.mask_preprocessing(prediction, i)

        fig, axes = plt.subplots(1, 1, figsize=(10, 12))

        axes.axis("off")
        #axes.set_title(id.split("_")[-1] + str("_"+str(i)), fontsize=35, weight='bold')
        axes.imshow(image, cmap='gray', alpha=1)
        axes.imshow(np.ma.masked_where(gt_mask_WT == False, gt_mask_WT),
                       cmap='YlOrBr', alpha=0.6)
        axes.imshow(np.ma.masked_where(gt_mask_TC == False, gt_mask_TC),
                       cmap='nipy_spectral', alpha=0.5)
        axes.imshow(np.ma.masked_where(gt_mask_ET == False, gt_mask_ET),
                       cmap='cool', alpha=0.5)
        print(id)
        if id=="BraTS20_Training_164":
           plt.savefig("ground_truth_transformerFedDy.png", bbox_inches='tight')

        fig2, axes2 = plt.subplots(1, 1, figsize=(10, 12))

        #axes2.set_title("Prediction "+str(id), fontsize=35, weight='bold')
        axes2.axis("off")
        axes2.imshow(image, cmap='gray', alpha=1)
        # axes2.imshow(np.ma.masked_where(pr_mask_WT == False, pr_mask_WT),
        #                cmap='Wistia', alpha=0.6)
        # axes2.imshow(np.ma.masked_where(pr_mask_TC == False, pr_mask_TC),
        #                cmap='rainbow', alpha=0.5)
        # axes2.imshow(np.ma.masked_where(pr_mask_ET == False, pr_mask_ET),
        ...
        plt.tight_layout()
        if id == "BraTS20_Training_164":
          plt.savefig("prediction_transformerFedDy.png", bbox_inches='tight')
...
class GlobalConfig:
    ...
    train_root_dir = 'C://Users//NEW//Desktop//FedTumor//FedTumor//BRATS//Data//Brats2020//MICCAI_BraTS2020_TrainingData'
...
def sample_test():
    ...
    ax0.imshow(sample_img[:, :, 65], cmap='gray', alpha=1)
    ax0.imshow(mask_WT[:, :, 65], cmap='cividis', alpha=0.4)
    ax0.imshow(np.ma.masked_where(mask_TC[:, :, 65] == False, mask_TC[:, :, 65]), cmap='rainbow', alpha=0.4)
    ax0.imshow(np.ma.masked_where(mask_ET[:, :, 65] == False, mask_ET[:, :, 65]), cmap='winter', alpha=0.4)
...
def get_augmentations(phase):
    ...
    # Disable the shape check if you are confident that the data is consistent
...
def plot_seg(model, dataloader, save_path):
    ...
    with torch.no_grad():
        for i, data in enumerate(dataloader):
            img, gt = data['image'].float(), data['mask'].float()
            id = str(data['Id'][0])
            ...
            show_result = ShowResult()
            show_result.plot(img, gt, logits, id)
...
def evaluate_trained_model():
    ...
    print("Valid length ", len(val_dataloader))



    global_model = UNet3d(in_channels=4, n_classes=3, n_channels=24).to(device)

    global_model.load_state_dict(torch.load("C://Users//NEW//Desktop//135//last_FedperAvg_round_model.pth", map_location=device))
    global_model.eval()

    plot_seg(global_model, val_dataloader, "C://Users//NEW//Desktop//FedTumor//FedTumor//")
