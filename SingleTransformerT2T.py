...
class T2TViTBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.adjust_channels = nn.Conv2d(in_channels, 3, kernel_size=1)
        self.t2t_vit = timm.create_model('t2t_vit_t_14', pretrained=False)
        self.t2t_vit.head = nn.Identity()  # Remove the classification head

        # Assuming the T2T-ViT does not change spatial resolution,
        # only the number of channels (features).
        self.conv1x1 = nn.Conv3d(768, out_channels, kernel_size=1)
    ...
    def forward(self, x):
        ...
        # Apply the 2D T2T-ViT to each slice
        # Resize each slice to the expected input size of the T2T-ViT
...
class UNet3d(nn.Module):
    def __init__(self, in_channels, n_classes, n_channels):
        ...
        # T2T-ViT block
        self.t2t_vit_block = T2TViTBlock(8 * n_channels, 8 * n_channels)
    ...
    def forward(self, x):
        ...
        # Apply T2T-ViT block
        t2t_vit = self.t2t_vit_block(x5)

        mask = self.dec1(t2t_vit, x4)
