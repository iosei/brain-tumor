...
class SwinTransformerBlock(nn.Module):
    def __init__(self, dim, in_channels, out_channels, num_heads=8, window_size=7):
        super(SwinTransformerBlock, self).__init__()
        self.window_size = window_size

        self.conv1 = nn.Conv3d(in_channels, out_channels, kernel_size=1)
        self.conv2 = nn.Conv3d(out_channels, out_channels, kernel_size=window_size, padding=window_size // 2)
        self.norm = nn.LayerNorm([dim, dim, dim])
    ...
    def forward(self, x):
        x = self.conv1(x)
        x = Rearrange('b c z (h p1) (w p2) -> b (z h w) (p1 p2 c)', p1=self.window_size, p2=self.window_size)(x)
        x = self.conv2(x)
        x = Rearrange('(b z) (h w) (p1 p2 c) -> b c z h w', p1=self.window_size, p2=self.window_size)(x)
        x = self.norm(x)
        x = self.mlp(x)
        return x

class SwinTransformerStage(nn.Module):
    def __init__(self, dim, in_channels, out_channels, num_blocks, num_heads=8, window_size=7):
        super(SwinTransformerStage, self).__init__()
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim, in_channels, out_channels, num_heads=num_heads, window_size=window_size)
...
class UNet3dWithSwin(nn.Module):
    def __init__(self, in_channels, n_classes, n_channels, swin_dim=16, num_heads=8, window_size=7, swin_num_blocks=1):
        super(UNet3dWithSwin, self).__init__()
        ...
        # Replace the first convolutional layer with Swin Transformer
        self.swin_conv = SwinTransformerBlock(swin_dim, n_channels, n_channels, num_heads=num_heads, window_size=window_size)
    ...
    def forward(self, x):
        x1 = self.conv(x)
        # Apply Swin Transformer block to the first convolutional layer
        x1 = self.swin_conv(x1)
...
def train_val_3unet(dataloader):
    ...
    # with
    nodel = UNet3dWithSwin(in_channels=4, n_classes=3, n_channels=24).to(device)
    # %%
