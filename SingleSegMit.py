...
class MITBlock(nn.Module):
    def __init__(self, in_channels, out_channels, img_size=224):
        ...
        # MIT model with a specified configuration
        self.mit_model = timm.create_model('mixer_b16_224', pretrained=False)
        self.mit_model.head = nn.Identity()  # Remove the classification head
    ...
    def forward(self, x):
        ...
        # Apply the 2D Mixed Image Transformer to each slice
        x = F.interpolate(x, size=(224, 224), mode='bilinear', align_corners=False)
        x = self.adjust_channels(x)
        x = self.mit_model(x)
        print("mit shape x", x.shape)
...
class UNet3d(nn.Module):
    def __init__(self, in_channels, n_classes, n_channels):
        ...
        # Replace the ViTBlock with MITBlock
        self.mit_block = MITBlock(8 * n_channels, 8 * n_channels)
    ...
    def forward(self, x):
        ...
        x4 = self.enc3(x3)
        x5 = self.enc4(x4)
        print("shape of x5", x5.size())

        # Apply MITBlock instead of ViTBlock
        mit_output = self.mit_block(x5)
        print("encoder", mit_output.size())

        mask = self.dec1(mit_output, x4)
