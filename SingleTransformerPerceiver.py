...
class PerceiverBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        config = PerceiverConfig(
            input_channels=3,
            input_axis=2,, out_channels, kernel_size=1)
    ...
    def forward(self, x):
        ...
        # Apply the 2D Perceiver to each slice
        # Resize each slice to the expected input size of the Perceiver
        x = F.interpolate(x, size=(224, 224), mode='bilinear', align_corners=False)
        x = self.adjust_channels(x)

        # Flatten the image for Perceiver input
        x = x.view(b * d, 3, -1).permute(0, 2, 1)  # (batch, sequence_length, channels)
        x = self.perceiver(inputs=x).last_hidden_state

        # Reshape back to original dimensions
        x = x.permute(0, 2, 1).view(b * d, 1024, 14, 14)  # (batch * depth, channels, height, width)
        x = F.interpolate(x, size=(d, h, w), mode='trilinear', align_corners=False)
        x = x.view(b, d, 1024, h, w)
...
class UNet3d(nn.Module):
    def __init__(self, in_channels, n_classes, n_channels):
        ...
        # Perceiver Transformer block
        self.perceiver_block = PerceiverBlock(8 * n_channels, 8 * n_channels)
    ...
    def forward(self, x):
        ...
        x4 = self.enc3(x3)
        x5 = self.enc4(x4)
        # Apply Perceiver Transformer block
        perceiver = self.perceiver_block(x5)

        mask = self.dec1(perceiver, x4)
