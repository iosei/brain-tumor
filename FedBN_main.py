...
def train_val_3unet():
    ...
    for rnd in range(number_of_rounds):
        ...
        round_dice_list = []
        round_iou_list = []
        print("FedBN training for round ",str(rnd + 1))
        for client in range(number_of_clients):

            print("Currently training for client ", str(client+1))
            current_client_data = clients_list_space[client]
            model_instance = copy.deepcopy(global_model)
            model_instance.load_state_dict(clients_init_params[client])
            trainer = Trainer(net=model_instance,
        ...
        clients_init_params, w_glob= FedBn(copy.deepcopy(clients_init_params), copy.deepcopy(global_model.state_dict()))
        ...
        df.to_csv("FedBN_training.csv", index=False)
    ...
    fig.savefig("fedBN_result1.png", format="png", pad_inches=0.2, transparent=False, bbox_inches='tight')
    fig.savefig("fedBN_result1.svg", format="svg", pad_inches=0.2, transparent=False, bbox_inches='tight')
    ...
    title = "Ground Truth_fedBN" + id_[0]
    ...
    merging_two_gif(filename1,
                    filename2,
                    'resultBN.gif')
    show_gif('resultBN.gif', format='png')
...
if __name__ == "__main__":
    ...
    survival_info_df = pd.read_csv(
        '/home/zengsn/BRATS/Data/Brats2020/BraTS2020_TrainingData/MICCAI_BraTS2020_TrainingData/survival_info.csv')
    name_mapping_df = pd.read_csv(
        '/home/zengsn/BRATS/Data/Brats2020/BraTS2020_TrainingData/MICCAI_BraTS2020_TrainingData/name_mapping.csv')
