...
class BratsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.phase != "test":
            ...
            augmented = self.augmentations(image=img.numpy(), mask=mask.astype(np.float32))
    ...
    def load_img(self, file_path):
        ...
        if data.size == 0:
            raise ValueError(f"Empty data loaded from {file_path}")
        return data
...
def evaluate_trained_model():
    ...
    global_model.load_state_dict(torch.load("C:/Users/NEW/Desktop/FedTumor/FedTumor/New_folder/last_epoch_model.pth", map_location=device))

    #global val metrics
    global_model.eval()
    #val_loss, val_dice, val_iou = compute_val_metrics(global_model, val_dataloader)
    no_sample_data = len(val_dataloader)
    print(f"No. of samples in val_dataloader: {no_sample_data}")
    # Example: Add this before any sampling call

    if val_dataloader=="":
        print("empty val_dataloader")
    dice_scores_per_classes, iou_scores_per_classes = compute_scores_per_classes(
        global_model, val_dataloader, ['WT', 'TC', 'ET']
    )

    # %%
    if dice_scores_per_classes=="":
        print("empty dice_scores_per_classes")
    else:
        print(dice_scores_per_classes)
    dice_df = pd.DataFrame(dice_scores_per_classes)
    print("Dice DF shape:", dice_df.shape)

    dice_df.columns = ['WT dice', 'TC dice', 'ET dice']
    if iou_scores_per_classes=="":
        print("empty iou_scores_per_classes")
    else:
        print(iou_scores_per_classes)

    iou_df = pd.DataFrame(iou_scores_per_classes)
    print("IoU DF shape:", iou_df.shape)
    print(iou_df)
    ...
    fig.savefig("singleno_result1.png", format="png", pad_inches=0.2, transparent=False, bbox_inches='tight')
    fig.savefig("singleno_result1.svg", format="svg", pad_inches=0.2, transparent=False, bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    name_mapping_df = pd.read_csv(
        'C://Users//NEW//Desktop//FedTumor//FedTumor//BRATS//Data//Brats2020//BraTS2020_ValidationData//MICCAI_BraTS2020_ValidationData//name_mapping_validation_data.csv')
