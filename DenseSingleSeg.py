...
class DenseTransformerBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.norm = nn.GroupNorm(num_groups=out_channels, num_channels=out_channels)

    def forward(self, x):
        x1 = F.relu(self.norm(self.conv1(x)))
        x2 = F.relu(self.norm(self.conv2(x1)))
...
class Down(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.encoder = nn.Sequential(
            nn.MaxPool3d(2, 2),
            DenseTransformerBlock(in_channels, out_channels)
        )
...
class Up(nn.Module):
    def __init__(self, in_channels, out_channels, trilinear=True):
        ...
        self.conv = DenseTransformerBlock(in_channels, out_channels)
...
class DenseTAN(nn.Module):
    def __init__(self, in_channels, n_classes, n_channels):
        super(DenseTAN, self).__init__()
        self.in_channels = in_channels
        self.n_classes = n_classes
        self.n_channels = n_channels

        self.conv = DenseTransformerBlock(in_channels, n_channels)
...
def train_val_3unet(dataloader):
    ...
    nodel = DenseTAN(in_channels=4, n_classes=3, n_channels=24).to(device)
    # %%
