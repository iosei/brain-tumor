...
class Image3dToGIF3d:
    """
    Displaying 3D images in 3D axes and optionally creating rotating GIFs.

    Parameters:
        img_dim (tuple): Shape of the cube for resizing (default: (55, 55, 55)).
        figsize (tuple): Figure size for plotting in inches (default: (15, 10)).
        binary (bool): If True, apply a binary colormap (default: False).
        normalizing (bool): If True, normalize the data before plotting (default: True).
    """
    ...
    def __init__(self,
        ...
        print(f"Image dimensions set to: {img_dim}")
    ...
    def _explode(self, data: np.ndarray):
        """
        Expand the voxel grid by inserting zeros between voxels to create space.

        Parameters:
            data (np.ndarray): Original voxel data with shape (x, y, z, 4).

        Returns:
            np.ndarray: Exploded voxel data with shape (2x-1, 2y-1, 2z-1, 4).
        """
        shape_arr = np.array(data.shape)
        size = shape_arr[:3] * 2 - 1
        exploded_shape = tuple(size) + (shape_arr[3],)
        exploded = np.zeros(exploded_shape, dtype=data.dtype)
        exploded[::2, ::2, ::2] = data
        print(f"Exploded shape: {exploded.shape}")
        return exploded

    def _expand_coordinates(self, indices: np.ndarray):
        """
        Adjust the coordinates to align with the exploded voxel grid.

        Parameters:
            indices (np.ndarray): Grid indices with shape (3, x*2, y*2, z*2).

        Returns:
            tuple: Adjusted x, y, z coordinate arrays each with shape (x*2, y*2, z*2).
        """
        x, y, z = indices
        x[1::2, :, :] += 1
        y[:, 1::2, :] += 1
        z[:, :, 1::2] += 1
        print(f"Coordinates expanded to shape: {x.shape}, {y.shape}, {z.shape}")
        return x, y, z
    ...
    def _normalize(self, arr: np.ndarray):
        ...
        if arr_max - arr_min == 0:
            print("Normalization skipped: array has zero variance.")
            return np.zeros_like(arr)
        normalized = (arr - arr_min) / (arr_max - arr_min)
        print(f"Data normalized. Min: {normalized.min()}, Max: {normalized.max()}")
        return normalized

    def _scale_by(self, arr: np.ndarray, factor: float):
        """
        Scale the 3D image by a specified factor.

        Parameters:
            arr (np.ndarray): 3D image for scaling.
            factor (float): Factor for scaling.

        Returns:
            np.ndarray: Scaled 3D image.
        """
        mean = np.mean(arr)
        scaled = (arr - mean) * factor + mean
        print(f"Data scaled by a factor of {factor}.")
        return scaled

    def get_transformed_data(self, data: np.ndarray):
        """Data transformation: normalization, scaling, resizing."""
        if self.binary:
            resized_data = resize(data, self.img_dim, preserve_range=True, anti_aliasing=False)
            transformed_data = np.clip(resized_data.astype(np.uint8), 0, 1).astype(np.float32)
            print(f"Transformed binary data shape: {transformed_data.shape}")
            return transformed_data

        norm_data = self._normalize(data)
        norm_data = np.clip(norm_data - 0.1, 0, 1) ** 0.4
        scaled_data = self._scale_by(norm_data, 2)
        scaled_data = np.clip(scaled_data - 0.1, 0, 1)
        resized_data = resize(scaled_data, self.img_dim, preserve_range=True, anti_aliasing=False)
        print(f"Transformed non-binary data shape: {resized_data.shape}")
        return resized_data
    ...
    def plot_cube(self,
        ...
        Parameters:
            cube (np.ndarray): 3D data.
            title (str): Title for the figure.
            init_angle (int): Initial angle for the 3D plot (from 0-360 degrees).
            make_gif (bool): If True, create a GIF by rotating the plot and capturing frames.
            path_to_save (str): Path to save the GIF file.
        """
        # Validate input cube
        ...
        # Apply colormap
        if self.binary:
            facecolors = cm.winter(cube)  # Binary colormap
            print("Using binary colormap (winter).")
        else:
            if self.normalizing:
                cube = self._normalize(cube)
                print("Data normalized.")
            facecolors = cm.gist_stern(cube)  # Non-binary colormap
            print("Using non-binary colormap (gist_stern).")

        # Ensure cube and facecolors have matching spatial dimensions
        if cube.shape != facecolors.shape[:3]:
            raise ValueError(f"Shape mismatch: cube shape {cube.shape} vs facecolors shape {facecolors.shape[:3]}")

        # Assign the alpha channel based on cube intensity
        facecolors[:, :, :, -1] = cube
        print("Alpha channel assigned based on cube intensity.")

        # Explode the facecolors to create space between voxels
        facecolors = self._explode(facecolors)

        # Create a filled mask where alpha channel is non-zero
        filled = facecolors[:, :, :, -1] != 0
        print(f"Filled mask created with shape: {filled.shape}")

        # Generate voxel coordinates
        indices = np.indices(np.array(filled.shape) + 1)
        x, y, z = self._expand_coordinates(indices)

        # Sanity check to ensure coordinate arrays match expected shape
        ...
        # Ensure facecolors shape matches filled mask
        if facecolors.shape[:3] != filled.shape:
            raise ValueError(f"Shape mismatch between facecolors and filled mask: "
                             f"facecolors {facecolors.shape[:3]} vs filled {filled.shape}")
        ...
        with plt.style.context("dark_background"):
            ...
            print("Voxels plotted.")

            if make_gif:
                images = []
                print("Starting GIF creation...")
                for angle in tqdm(range(0, 360, 5), desc="Creating GIF"):
                ...
                # Save the GIF
                imageio.mimsave(path_to_save, images, fps=10)
                plt.close(fig)
                print(f"GIF saved to {path_to_save}")
            else:
                plt.show()
...
# Example usage
# merging_two_gif('path_to_ground_truth.gif', 'path_to_prediction.gif', 'merged_output.gif')
...
def accuracy_metric(probabilities: torch.Tensor,
    ...
    """
    Calculate accuracy for a data batch.
    ...
    for i in range(num):
        prediction = predictions[i]
        truth_ = truth[i]
        correct = (prediction == truth_).float().sum()
        total = truth_.numel()
...
def accuracy_metric_per_classes(probabilities: np.ndarray,
                                truth: np.ndarray,
                                threshold: float = 0.5,
                                classes: list = ['WT', 'TC', 'ET']) -> dict:
    """
    Calculate accuracy for each class in a data batch.
    ...
    Returns:
        Dictionary with accuracy scores for each class.
    """
    ...
    for i in range(num):
        for class_ in range(num_classes):
            ...
            correct = (prediction == truth_).sum()
            total = truth_.size
            scores[classes[class_]].append(correct / total)

    return scores


class Meter:
    '''Factory for storing and updating evaluation metrics.'''
    ...
    def update(self, logits: torch.Tensor, targets: torch.Tensor):
        """
        Takes logits from the model and targets, calculates Dice, IoU,
        and Accuracy scores, and stores them in lists.
        """
        probs = torch.sigmoid(logits)
        dice = dice_coef_metric(probs, targets, self.threshold)
        iou = jaccard_coef_metric(probs, targets, self.threshold)
        accuracy = accuracy_metric(probs, targets, self.threshold)
    ...
    def get_metrics(self) -> tuple:
        """
        Returns the average of accumulated Dice, IoU, and Accuracy scores.
        """
...
class Trainer:
    ...
    def __init__(self,
        ...
        self.phases = ["train", "val"]  # Ensure this is defined early
        ...
        self.accuracy_scores = {phase: [] for phase in self.phases}  # Add accuracy tracking
    ...
    def _do_epoch(self, epoch: int, phase: str):
        ...
        for itr, data_batch in enumerate(dataloader):
            ...
            loss, logits, accuracy = self._compute_loss_and_outputs(images, targets)
        ...
        epoch_dice, epoch_iou, epoch_accuracy = meter.get_metrics()

        self.losses[phase].append(epoch_loss)
        self.dice_scores[phase].append(epoch_dice)
        self.jaccard_scores[phase].append(epoch_iou)
        self.accuracy_scores[phase].append(epoch_accuracy)  # Track accuracy

        return epoch_loss, epoch_dice, epoch_iou, epoch_accuracy
    ...
    def _compute_loss_and_outputs(self, images: torch.Tensor, targets: torch.Tensor):
        ...
        # Calculate accuracy
        preds = torch.argmax(logits, dim=1)
        correct = (preds == targets).sum().float()  # Use float for division
        total = targets.numel()
    ...
    def _plot_train_history(self):
        data = [self.losses, self.dice_scores, self.jaccard_scores, self.accuracy_scores]
        colors = ['deepskyblue', "crimson"]
        labels = [
            f"Train loss: {self.losses['train'][-1]} | Val loss: {self.losses['val'][-1]}",
            f"Train dice: {self.dice_scores['train'][-1]} | Val dice: {self.dice_scores['val'][-1]}",
            f"Train Jaccard: {self.jaccard_scores['train'][-1]} | Val Jaccard: {self.jaccard_scores['val'][-1]}",
            f"Train accuracy: {self.accuracy_scores['train'][-1]} | Val accuracy: {self.accuracy_scores['val'][-1]}",
        ]
...
def compute_val_metrics(net, data):
    ...
    val_dice, val_iou, val_accuracy = meter.get_metrics()  # Unpack accuracy as well

    return val_loss, val_dice, val_iou, val_accuracy  # Return all metrics
...
def train_val_3unet():
    ...
    for rnd in range(number_of_rounds):
        ...
        for client in range(number_of_clients):
            ...
            client_w, client_loss, client_dice, client_iou, client_accuracy = trainer.run()
            ...
            round_accuracy_list.append(client_accuracy)
        ...
        # Calculate mean metrics for the current round
        round_loss = np.mean(round_loss_list)
        round_dice = np.mean(round_dice_list)
        round_iou = np.mean(round_iou_list)
        round_accuracy = np.mean(round_accuracy_list)  # Add mean accuracy calculation

        # global val metrics
        global_model.eval()
        gc.collect()
        torch.cuda.empty_cache()
        val_loss, val_dice, val_iou, val_accuracy = compute_val_metrics(global_model, val_dataloader)

        all_round_metrics.append(
            [rnd + 1, round_loss, round_dice, round_iou, round_accuracy, val_loss, val_dice, val_iou, val_accuracy])
        df = pd.DataFrame(all_round_metrics,
                          columns=['round', 'train_loss', 'train_dice', 'train_iou', 'train_accuracy',
                                   'val_loss', 'val_dice', 'val_iou', 'val_accuracy'])
        df.to_csv("FedperAvg_training.csv", index=False)

        print(
            "Round {}: Train loss {}, Train dice {}, Train iou {}, Train accuracy {}, Val loss {}, Val dice {}, Val iou {}, Val accuracy {} \n".format(
                rnd + 1, round_loss, round_dice, round_iou, round_accuracy, val_loss, val_dice, val_iou, val_accuracy))
