...
class GlobalConfig:
    root_dir = 'C:/Users/HP/PycharmProjects/FedTumor/BRATS/Data/Brats2020/BraTS2020_TrainingData'
    ...
    pretrained_model_path = 'C:/Users/HP/PycharmProjects/FedTumor/BRATS/Data/Brats2020/BraTS2020_TrainingData/brats2020logs/last_epoch_model.pth'
    train_logs_path = 'C:/Users/HP/PycharmProjects/FedTumor/BRATS/Data/Brats2020/BraTS2020_TrainingData/brats2020logs/train_log.csv'
    ae_pretrained_model_path = 'C:/Users/HP/PycharmProjects/FedTumor/BRATS/Data/Brats2020/BraTS2020_TrainingData/brats2020logs/autoencoder_best_model.pth'
    tab_data = 'C:/Users/HP/PycharmProjects/FedTumor/BRATS/Data/Brats2020/BraTS2020_TrainingData/brats2020logs/df_with_voxel_stats_and_latent_features.csv'
    seed = 55
...
class ImageGPTBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.adjust_channels = nn.Conv2d(in_channels, 1, kernel_size=1)

        self.learn_channels = nn.Conv2d(1, 192, kernel_size=5, stride=3)

        # Initialize GPT2 model
        self.config = GPT2Config(n_positions=1024, n_ctx=1024, n_embd=768, n_layer=12, n_head=12)
        self.image_gpt = GPT2Model(self.config)

        # Linear layer to map image pixels to the model's embedding size
        self.linear = nn.Linear(15 * 15 * 9,self.config.n_positions * self.config.n_embd)
    ...
    def forward(self, x):
        ...
        # Apply the 2D Image GPT to each slice
        # Resize each slice to the expected input size of the Image GPT
        #x = F.interpolate(x, size=(9, 9), mode='bilinear', align_corners=False)
        x = self.adjust_channels(x)

        x = x.view(1, -1)
        # Flatten the image for GPT input

        x = self.linear(x)
        # Reshape embeddings to (batch_size, sequence_length, embedding_size)
        x = x.view(1, self.config.n_positions ,  self.config.n_embd)
        # Add position ids
        position_ids = torch.arange(0, self.config.n_positions, dtype=torch.long, device=device)
        position_ids = position_ids.unsqueeze(0).expand(1, -1)

        x= x.to(torch.long)
        x = self.image_gpt(inputs_embeds=x,position_ids=position_ids).last_hidden_state

        # Reshape back to original dimensions
        x = x.view(1,1,1024, 768)
        x = self.learn_channels(x)
        x= x.unsqueeze(2)
...
class UNet3d(nn.Module):
    def __init__(self, in_channels, n_classes, n_channels):
        ...
        # Image GPT block
        self.image_gpt_block = ImageGPTBlock(8 * n_channels, 8 * n_channels)
    ...
    def forward(self, x):
        ...
        x4 = self.enc3(x3)
        x5 = self.enc4(x4)
        # Apply Image GPT block
        imgpt = self.image_gpt_block(x5)
        print(imgpt.shape)
        mask = self.dec1(imgpt, x4)
